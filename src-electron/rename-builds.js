...
function renameBuild(arch, platform) {
    if (platform === 'linux') {
        const oldAppImage = path.join(buildDir, `XCRV_Version.AppImage`);
        const newAppImage = path.join(
            buildDir,
            `XCRV_${version}_${arch}.AppImage`
        );
        try {
        ...
        const oldDmg = path.join(buildDir, `XCRV_Version.dmg`);
        const newDmg = path.join(buildDir, `XCRV_${version}_${arch}.dmg`);
        try {
